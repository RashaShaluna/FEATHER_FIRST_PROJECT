...
const offerActive = async (req, res) => {
    try {
        ...
        if (
            ...
            console.log("Product with ID", productId, "does not have a valid offer percentage or dates");
    ...
    try {
       ...
       log("Activating offer for category with ID", categoryId);
        ...
        if (!category || !category.offerPercentage || category.offerPercentage <= 0) {
           log("Category with ID", categoryId, "does not have a valid offer percentage");
            return res.redirect(`/admin/category?error=missing-percentage`);
        }

        if (category) {
           log("Updating offer price for all products in category with ID", categoryId);
